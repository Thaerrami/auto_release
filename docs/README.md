# Release Tool

**Interactive CLI for Git release management** across the Literatum UI widget repo hierarchy. Handles cherry-picks, dependency bumps, builds, tagging, and pushing in dependency order — with crash recovery and concurrency locking.

Replaces the legacy `codeUpdate.sh`, `upgradeArticle.sh`, and `UpgradeTheme2.sh` scripts.

## What it does

- **Auto-detects** the repo you’re in (or use `--repo <id>`) and resolves the full **dependency tree** to release (parent → children).
- For each repo, per **version track** (e.g. `v2.7`, `v2.8`):
  - Syncs from `develop`, checks out latest tag in track.
  - **Cherry-picks** commits you specify (shared across repos/tracks).
  - **Bumps parent deps** in `package.json` to the new tag (or latest remote).
  - Optional **npm install** / **npm run build** with retry/skip.
  - **Diff summary** and explicit **[P]ush** confirmation before tagging and pushing.
- **Run once**: choose tracks and cherry-pick SHAs at the start; they apply to all repos in the run.
- **Logs** every run to `release-logs/` (`.log` + `.json`). Supports **crash recovery** and a **file lock** to avoid concurrent releases.
- **Upgrades ui-products** after a successful release — finds products (and nested products) whose theme/core/article deps match the upgraded versions, then offers to bump them on `develop`.

Two interfaces:

| Interface | Description |
|-----------|-------------|
| **Node (recommended)** | `node dist/index.js` or `npm start` — interactive prompts, crash recovery, lock, structured logs. |
| **Shell** | `release.sh` — same workflow in bash; use from a repo dir or `--repo <id>`. |

## Prerequisites

- Node.js 18+
- All repos cloned as siblings in the same parent directory:

```
ui-workspace/
├── auto_release/     <-- this tool
├── ui-base/
├── ui-core/
├── ui-theme-photo/
├── ui-theme-classic/
├── ui-theme-eureka/
├── ui-theme-nextgen/
├── ui-products/      <-- product repos (post-release dep upgrades)
└── ui-article/
```

## Install & Build

```bash
cd auto_release
npm install
npm run build
```

## Usage

> **Quick reference:** see **[COMMANDS.md](COMMANDS.md)** for a one-page command sheet (common recipes, all flags, interactive prompts).

### Command-line help

Print all flags and examples (then exit):

```bash
node dist/index.js --help
# or
node dist/index.js -h
npm start -- --help
```

### Run from inside a repo (recommended)

`cd` into the repo you want to release from. The tool auto-detects where you are and resolves the full dependency tree downward:

```bash
cd ../ui-core
node /path/to/auto_release/dist/index.js
```

| You stand in | Tool processes |
|---|---|
| `ui-base` | ui-base, ui-core, ui-theme-eureka, ui-theme-photo, ui-theme-classic |
| `ui-core` | ui-core, ui-theme-photo, ui-theme-classic |
| `ui-theme-photo` | ui-theme-photo |
| `ui-theme-eureka` | ui-theme-eureka |
| `ui-article` | ui-article |

### Run with `--repo` flag

From any directory, specify the starting repo explicitly:

```bash
node dist/index.js --repo ui-base
```

### Dry run (preview only)

See what would happen without writing anything:

```bash
node dist/index.js --repo ui-core --dry-run
```

### Shell script (`release.sh`)

Same workflow in bash. Run from inside a repo or pass `--repo`:

```bash
cd ../ui-core
bash /path/to/auto_release/release.sh
```

```bash
bash release.sh --repo ui-base --dry-run
```

| Flag | Description |
|------|-------------|
| `--dry-run` | No writes; prints what would run. |
| `--verbose` | Extra output. |
| `--repo <id>` | Starting repo (e.g. `ui-base`, `ui-core`). |

The Node tool also supports `-h` / `--help` and automation flags; see [CLI Flags](#cli-flags).

Logs go to `release-logs/release-YYYYMMDD-HHMMSS.log`.

## PowerPoint

A slide deck is generated by:

```bash
npm run ppt
```

This creates **Release-Tool.pptx** in the project root (overview, repo tree, flow, usage, CLI flags, product upgrades, logging, demo).

## Demo

To try the tool without changing any repo, run a **dry run** from a repo directory (or use `--repo`). See **[DEMO.md](DEMO.md)** for a step-by-step walkthrough and example output.

```bash
cd /path/to/ui-workspace/ui-core
node /path/to/auto_release/dist/index.js --repo ui-core --dry-run
```

You’ll be prompted for tracks and cherry-pick SHAs; all writes are skipped and shown as `[DRY-RUN]`.

## CLI Flags

| Flag | Description |
|---|---|
| `-h`, `--help` | Print usage, all options, and examples; then exit (no prompts, no lock). |
| `--dry-run` | Preview mode. All reads execute, all writes are skipped and printed as `[DRY-RUN]`. |
| `--verbose` | Print full raw git and npm command output. |
| `--no-color` | Disable ANSI colors (useful for piping output or CI). |
| `--repo <id>` | Override CWD auto-detection. Must be one of: `ui-base`, `ui-core`, `ui-theme-photo`, `ui-theme-classic`, `ui-theme-eureka`, `ui-article`. |
| `--log-dir <path>` | Custom log output directory. Default: `./release-logs/`. |
| `--lock-path <path>` | Custom lock file path. Default: `/tmp/release-tool-<hostname>.lock`. |
| `--skip-install-build` | Skip **npm/yarn install** and **npm/yarn build** for every repo and track, with no Yes/Skip prompts. |
| `--auto-push` | After each **diff summary** (summary is still printed), **push** immediately without the [P]ush / [S]kip repo / [A]bort all menu. |
| `--skip-product-upgrade` | Skip the **post-release ui-products** dependency upgrade step entirely. |
| `--auto-upgrade-products` | After release, upgrade **all affected products** without the selection prompt (still stashes/checkouts `develop`). |
| `--skip-product-install` | When upgrading products, skip `npm install` in each product directory (faster; lockfiles may be stale). |

### Less interactive runs

The tool is interactive by default (install/build prompts, push confirmation). For a faster path when you already trust the tree:

- **Skip install and build everywhere:** `--skip-install-build`
- **Push right after each diff summary:** `--auto-push`
- **Combine both** (fewer stops; you still answer tracks, cherry-picks, conflicts, lock, etc.):

```bash
node dist/index.js --skip-install-build --auto-push
```

- **Full hotfix with automatic product upgrades:**

```bash
node dist/index.js --repo ui-theme-photo --auto-push --auto-upgrade-products
```

- **Release only, skip product step:**

```bash
node dist/index.js --repo ui-core --skip-product-upgrade
```

`--auto-push` does **not** run install/build for you; it only skips the post-summary push menu. Use `--skip-install-build` if you want to skip those steps entirely, or answer the install/build prompts when you omit it.

## Repo Dependency Tree

```
ui-base (layer 1)
├── ui-core (layer 2)
│   ├── ui-theme-photo (layer 3)
│   ├── ui-theme-classic (layer 3)
│   └── ui-theme-nextgen (layer 3)
└── ui-theme-eureka (layer 2)

ui-products (layer 3, no tags — upgraded after release)
ui-article (layer 0, independent versioning)
```

All `main`-versioned repos share the same `major.minor` track (e.g. `v2.7`). Only the patch number increments per release. `ui-article` has its own independent version namespace.

## Per-Repo Release Flow

For each repo in dependency order, the tool runs these steps:

1. **Checkout & sync** -- `git checkout develop`, `git pull`, `git fetch --tags`
2. **Dirty tree check** -- If uncommitted changes exist, offers to stash, proceed, or abort
3. **Track selection** -- Lists all `major.minor` tracks with their latest tag. Select one or more tracks, or enter a new track manually
4. **Per-track processing** (for each selected track):
   - **Compute next tag** -- Finds latest patch in the track, increments by 1
   - **Checkout tag** -- Checks out the latest tag for the track (detached HEAD)
   - **Bump parent dep** -- If a parent repo was released in this run (or has a remote tag on this track), updates `package.json` with the exact new version
   - **Cherry-pick** -- Prompts for commit SHAs to cherry-pick. **Merge commits** are detected and applied with `-m 1` automatically (see [Cherry-pick and merge commits](#cherry-pick-and-merge-commits)). Validates each SHA exists. Pauses on conflicts for manual resolution
   - **npm install** -- Runs install (optional skip per repo unless `--skip-install-build`). On failure, offers retry, skip, or abort
   - **npm run build** -- Runs build (optional skip per repo unless `--skip-install-build`). On failure, offers retry, skip, or abort
   - **Diff summary** -- Shows a boxed summary of commits, file stats, dep changes, and the tag to create. Normally requires explicit `[P]ush` confirmation; use `--auto-push` to push without that menu
   - **Push** -- Creates tag, pushes to origin. Handles non-fast-forward, auth errors, protected branches, and timeouts with retries
   - **Return to develop** -- Checks out the base branch before the next track

## Post-release: ui-products dependency upgrade

After the release loop completes successfully (tags pushed, no hard failures), the tool can **propagate new versions into ui-products**.

### When it runs

- At least one tag was created in the run.
- No repo ended with status `failed`.
- Not skipped via `--skip-product-upgrade`.

### What it does

1. **Collects upgraded versions** from the run (`ui-theme-photo`, `ui-theme-classic`, `ui-theme-eureka`, `ui-theme-nextgen`, `ui-core`, `ui-article`).
2. **Scans ui-products** — every top-level and nested product `package.json` (e.g. `cabi`, `sage/mal`, `marlin/lancet`).
3. **Matches by repo + track** — e.g. a hotfix on `ui-theme-photo` track `v2.6` finds all products pinning `ui-theme-photo#v2.6.x` (including four-part versions like `v2.6.68.1`).
4. **Interactive menu:**
   - **[A] Upgrade all** listed products
   - **[S] Select** products individually (checkbox)
   - **[N] Skip**
5. **Prepares ui-products:** stash uncommitted changes → `git checkout develop` → `git pull`.
6. **Upgrades each product:** updates `package.json`, optional `npm install`, one commit per product.
7. **Offers to push** ui-products to `origin/develop`.

### ui-article in products

Products get `ui-article` bumped when:

- `ui-article` was released in this run (new tag pushed), or
- You chose to upgrade ui-article during the theme/core release (`single` mode, or `per-track` with one unique version).

### Example

Hotfix on `ui-theme-photo` → tag `v2.6.73`:

| Product | Before | After |
|---------|--------|-------|
| `cabi` | `ui-theme-photo#v2.6.72` | `ui-theme-photo#v2.6.73` |
| `sup` | `ui-theme-photo#v2.6.71` | `ui-theme-photo#v2.6.73` |
| `siam` | *(classic v2.6 — not affected)* | — |

### Automation flags

| Flag | Effect |
|------|--------|
| `--auto-upgrade-products` | Upgrade all matched products without the selection menu |
| `--skip-product-install` | Skip `npm install` in each product |
| `--skip-product-upgrade` | Do not run this step at all |

Dry run previews product upgrades as `[DRY-RUN]` lines without writing files.

## Cherry-pick and merge commits

When the SHA you cherry-pick is a **merge commit** (e.g. “Merge branch 'fix/xyz' into develop”), Git requires **`-m 1`** so it knows which parent is the mainline. Without it you get: `Commit X is a merge but no -m option was given`.

This happens often in **ui-base** (and other repos) because fixes often land as merge commits. The release tool **detects merge commits** (two parents) and runs `git cherry-pick -m 1 <sha>` for them automatically, so you don’t need to pass `-m 1` yourself. For more detail see **[docs/cherry-pick-merge.md](docs/cherry-pick-merge.md)**.

## Release Scenarios

| Scenario | Example |
|---|---|
| **Head repo only** | Run from `ui-base`. Cherry-pick, build, tag, push on selected tracks. |
| **Head + children** | Run from `ui-base`. After ui-base completes, ui-core gets the dep bump automatically, then themes get theirs. |
| **Children only** | Run from `ui-theme-photo`. Bumps dep to the latest existing parent tag (via `ls-remote`), cherry-picks, builds, tags. |
| **ui-article** | Run from `ui-article`. Independent versioning, same flow. Consuming repos can get ui-article bumped during theme releases. |
| **Multi-track** | Select `v2.7` and `v2.8` for the same repo. Each track is processed sequentially. One track failing does not block others. |
| **Theme hotfix + products** | Run from `ui-theme-photo`. After tag push, tool finds all products on matching `ui-theme-photo` track and offers to upgrade them in ui-products. |
| **Core hotfix + products** | Run from `ui-core`. Cascades to photo/classic themes; products using matching `ui-core` or theme versions are offered at the end. |

## Logging

Each run produces two files in `release-logs/`:

- **`release-YYYYMMDD-HHMMSS.log`** -- Full human-readable log of all prompts, responses, git output, and errors.
- **`release-YYYYMMDD-HHMMSS.json`** -- Structured JSON with run metadata, per-repo results, tags created, dep bumps, and errors.

## Crash Recovery

If the tool crashes mid-run, it writes a `run-state.json` file to the log directory. On the next startup, it detects the incomplete run and offers:

- **Resume** -- Continue from the last completed step
- **Fresh** -- Discard the previous state and start over
- **View** -- Print what was completed before the crash

## Concurrency Lock

In non-dry-run mode, the tool acquires a file lock (`proper-lockfile`) to prevent two engineers from running releases simultaneously. If the lock is already held, you'll see who holds it and since when, with options to wait, force-override (for crash recovery), or abort.

## Project Structure

```
src/
├── index.ts          Entry point and main orchestrator
├── types.ts          All TypeScript interfaces
├── config.ts         Repo graph, tree detection, dependency resolution
├── cli.ts            CLI flag parsing
├── messages.ts       All user-facing strings
├── logger.ts         Dual-output logger (.log + .json)
├── git-client.ts     Git operations (interface + implementation via simple-git)
├── lock.ts           Concurrency lock (proper-lockfile)
├── run-state.ts      Crash recovery state management
├── version.ts        Tag parsing, track grouping, version math
├── dep-bump.ts       Parent dependency bumping in package.json
├── dep-utils.ts      Shared package.json dep helpers
├── cherry-pick.ts    Cherry-pick execution with conflict handling
├── build.ts          npm install / build with retry loops
├── diff-summary.ts   Pre-push diff display
├── push.ts           Push with error classification and retries
├── product-deps.ts   Post-release ui-products dependency scan and upgrade
├── repo-flow.ts      Per-repo release flow (all steps per track)
└── startup.ts        Path validation, repo resolution, crash recovery UI
```
