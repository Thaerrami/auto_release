...
chmod +x "$(dirname "$0")/upgradeArticle.sh"
if [ "$repo_name" = "ui-article" ]; then
    "$(dirname "$0")/upgradeArticle.sh" "${revisions[@]}" "${tags[@]}"
else
    ...
    if [ ${#tags[@]} -ne 0 ]; then
        ...
        chmod +x "$(dirname "$0")/UpgradeTheme2.sh"
        "$(dirname "$0")/UpgradeTheme2.sh" "current_application" "${tags[@]}"
