...
exports.repoReleaseFlow = repoReleaseFlow;
const inquirer_1 = __importDefault(require("inquirer"));
const chalk_1 = __importDefault(require("chalk"));
const messages_1 = require("./messages");
const version_1 = require("./version");
const dep_bump_1 = require("./dep-bump");
const cherry_pick_1 = require("./cherry-pick");
const build_1 = require("./build");
const diff_summary_1 = require("./diff-summary");
const push_1 = require("./push");
async function repoReleaseFlow(repo, context, gitClient, logger, stateManager) {
    const result = {
        repoId: repo.id,
        tracksProcessed: [],
        tagsCreated: [],
    ...
    console.log(messages_1.msg.processingRepo(repo.id));
    logger.info(`Processing repo: ${repo.id}`, { repo: repo.id });
    if (repo.id === 'ui-base') {
        console.log(messages_1.msg.uiBaseDirectWarning());
    ...
    if (!hasOrigin) {
        console.log(messages_1.msg.remoteNotSet(repo.id));
        const { setRemoteUrl } = await inquirer_1.default.prompt([{
                type: 'input',
                name: 'setRemoteUrl',
        ...
        if (setRemoteUrl) {
            if (!context.dryRun) {
                ...
                console.log(messages_1.msg.dryRunSkip(`git remote add origin ${setRemoteUrl}`));
            }
        }
        else {
    ...
    if (!context.dryRun) {
        ...
        console.log(messages_1.msg.dryRunSkip(`git checkout ${repo.baseBranch}`));
        console.log(messages_1.msg.dryRunSkip('git pull'));
        console.log(messages_1.msg.dryRunSkip('git fetch --tags'));
    }
...
async function handleDirtyTree(repo, context, gitClient, logger, stateManager) {
    ...
    console.log(messages_1.msg.dirtyActions());
    ...
    if (action === 'stash') {
        if (context.dryRun) {
            console.log(messages_1.msg.dryRunSkip('git stash'));
        }
        else {
...
async function selectVersionTracks(repo, gitClient, logger, forAllRepos = false) {
    const tags = await gitClient.tagList(repo.localPath);
    const trackMap = (0, version_1.groupTagsByTrack)(tags);
    const trackChoices = [];
    for (const [track, infos] of trackMap) {
        const latest = (0, version_1.getLatestInTrack)(infos);
        if (latest) {
            let date = 'unknown';
            try {
            ...
            trackChoices.push({
                name: messages_1.msg.trackInfo(track, latest.tag, date),
                value: track,
            });
        }
    }
    trackChoices.sort((a, b) => a.value.localeCompare(b.value));
    trackChoices.push({ name: messages_1.msg.newTrackOption(), value: '__new__' });
    console.log(forAllRepos ? messages_1.msg.trackSelectOnce() : messages_1.msg.trackSelect(repo.id));
    const { selectedTracks } = await inquirer_1.default.prompt([{
            type: 'checkbox',
            name: 'selectedTracks',
    ...
    for (const t of selectedTracks) {
        if (t === '__new__') {
            const { newTrack } = await inquirer_1.default.prompt([{
                    type: 'input',
                    name: 'newTrack',
...
async function processTrack(repo, track, context, gitClient, logger, stateManager) {
    const result = {
        track,
        tagCreated: null,
        cherryPicks: [],
    ...
    const prevTag = (0, version_1.getLatestInTrack)(trackInfos)?.tag ?? null;
    if (!prevTag && trackInfos.length === 0) {
        console.log(chalk_1.default.yellow(`  No existing patch versions found for ${track}. Starting from .0`));
        logger.info(`No existing tags for track ${track}`, { repo: repo.id, track });
    }
    let newTag = (0, version_1.computeNextTag)(track, trackInfos);
    const tagResolution = await resolveTagConflicts(repo, track, newTag, trackInfos, gitClient, context, logger);
    ...
    console.log(messages_1.msg.computedTag(track, newTag));
    ...
    if (prevTag) {
        if (!context.dryRun) {
            ...
            console.log(chalk_1.default.dim(`  Checked out ${prevTag}`));
        }
        else {
            console.log(messages_1.msg.dryRunSkip(`git checkout ${prevTag}`));
        }
    }
    ...
    if (repo.id === 'ui-article') {
        if (!context.dryRun) {
            try {
                ...
                console.log(chalk_1.default.yellow(`  [article] Created tag ${newTag} (before cherry-pick — legacy behavior)`));
            }
            catch (err) {
            ...
            console.log(messages_1.msg.dryRunSkip(`git tag ${newTag} (before cherry-pick — article legacy behavior)`));
        }
    }
    ...
    if (!stateManager.isStepCompleted(repo.id, track, 'cherry-pick')) {
        const cpResult = await (0, cherry_pick_1.performCherryPicks)(repo.localPath, repo.id, track, gitClient, context, logger);
    ...
    if (!stateManager.isStepCompleted(repo.id, track, 'dep-bump')) {
        const bumped = await (0, dep_bump_1.bumpParentDependency)(repo, track, context, gitClient, logger);
    ...
    if (!stateManager.isStepCompleted(repo.id, track, 'npm-install')) {
        const installResult = await (0, build_1.runInstall)(repo.localPath, repo.id, track, context, logger);
    ...
    if (!stateManager.isStepCompleted(repo.id, track, 'npm-build')) {
        const buildResult = await (0, build_1.runBuild)(repo.localPath, repo.id, track, context, logger);
    ...
    const diffAction = await (0, diff_summary_1.showDiffSummary)(repo, displayPrevTag, newTag, gitClient, context, logger);
    ...
    if (!stateManager.isStepCompleted(repo.id, track, 'push')) {
        ...
        const pushResult = await (0, push_1.pushChanges)(repo.localPath, repo.id, track, branch, newTag, gitClient, context, logger, repo.id === 'ui-article' // tagAlreadyCreated — skip re-tagging for article
        );
        if (pushResult.success || pushResult.manual) {
            ...
            if (!context.dryRun && !pushResult.manual) {
                try {
                    ...
                    console.log(chalk_1.default.dim(`  Deleted local tag: ${newTag}`));
                }
                catch (err) {
                ...
                console.log(messages_1.msg.dryRunSkip(`git tag -d ${newTag} (post-push cleanup)`));
            }
...
async function resolveTagConflicts(repo, track, computedTag, trackInfos, gitClient, context, logger) {
    const parsed = (0, version_1.parseTag)(computedTag);
    if (parsed && parsed.patch > 999) {
        console.log(messages_1.msg.patchWarning(parsed.patch));
        const { patchAction } = await inquirer_1.default.prompt([{
                type: 'list',
                name: 'patchAction',
                message: 'Choose action:',
                choices: [
    ...
    if (existsLocally || existsRemote) {
        ...
        console.log(messages_1.msg.tagExists(computedTag, location));
        const { tagAction } = await inquirer_1.default.prompt([{
                type: 'list',
                name: 'tagAction',
                message: 'Choose action:',
                choices: [
        ...
        if (tagAction === 'delete') {
            if (context.dryRun) {
                console.log(messages_1.msg.dryRunSkip(`git tag -d ${computedTag}`));
                if (existsRemote) {
                    console.log(messages_1.msg.dryRunSkip(`git push origin --delete ${computedTag}`));
                }
            }
            else {
                if (existsLocally) {
...
async function promptCustomVersion(track) {
    const { customTag } = await inquirer_1.default.prompt([{
            type: 'input',
            name: 'customTag',
...
async function restoreStash(repo, wasStashed, gitClient, logger) {
    if (!wasStashed)
        return;
    try {
        ...
        console.log(messages_1.msg.stashPopFailed(stashList));
        logger.warn('Stash pop failed', { repo: repo.id });
    }
}
//# sourceMappingURL=repo-flow.js.map